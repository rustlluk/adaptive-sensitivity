...
try: # ROS version
    from pyKUKA.visualizer import Visualizer
    from pyKUKA.gripper import Barrett
    from pyKUKA.utils import Config, URDF, Pose, CustomFormatter
except ImportError: # Non-ROS version
    from visualizer import Visualizer
    from gripper import Barrett
...
class pyKUKA(BulletClient):
    ...
    def __init__(self, config="default.yaml"):
        ...
        if self.config.log.log:
            self.file_logger = logging.getLogger("pykuka_file_logger")
    ...
    def init_robot(self):
        ...
        bhand_links = ["finger_tip_23_vhacd.obj", "finger_22_vhacd.obj", "knuckle_2_vhacd.obj",
                       "finger_tip_13_vhacd.obj", "finger_12_vhacd.obj", "knuckle_vhacd.obj",
                       "finger_tip_33_vhacd.obj", "finger_32_vhacd.obj", "palm_280_vhacd.obj"]
        bhand_links_ids = []
        for link in self.urdfs["robot"].links:
            if hasattr(link, "collision"):
                fname = os.path.basename(link.collision.geometry.mesh.filename)
                if fname in bhand_links:
                    link_id = self.find_link_id(os.path.basename(link.collision.geometry.mesh.filename), robot=robot)
                    bhand_links_ids.append(link_id)
        for a,b in itertools.combinations(bhand_links_ids, 2):
            self.setCollisionFilterPair(robot, robot, a, b, False)
        for link in links:
            if link.name == "table1":
    ...
    def move_position(self, joints, positions, wait=True, velocity=None, set_col_state=True, check_collision=True):
        ...
        for joint, position in zip(joints, positions):
            ...
            if not (self.joints[joint_id].lower_limit <= position <= self.joints[joint_id].upper_limit):
                ...
                # self.logger.warning(f"Joint {joint} position {position} as it is out of bounds "
                #                     f"({self.joints[joint_id].lower_limit}, {self.joints[joint_id].upper_limit}). Clipping to limits.")
                # continue
    ...
    def move_velocity(self, joints, velocities):
        ...
        for joint, velocity in zip(joints, velocities):
            ...
            if np.abs(velocity) > self.joints[joint_id].max_velocity:
                velocity = np.sign(velocity)*self.joints[joint_id].max_velocity
